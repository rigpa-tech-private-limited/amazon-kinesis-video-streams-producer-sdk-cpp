...
void kinesis_video_init(CustomData *data) {
    ...
    std::cout << "defaultRegionStr" << std::endl;
    std::cout << defaultRegionStr << std::endl;
...
int gstreamer_init(int argc, char *argv[]) {
    ...
    while (rtsp_url_file >> rtsp_url) {
        std::string kvs_rtsp_url = rtsp_url;
        std::string delimiter = "####";
        size_t pos = 0;
        std::string video_stream_input;
        while ((pos = kvs_rtsp_url.find(delimiter)) != std::string::npos) {
            video_stream_input = kvs_rtsp_url.substr(0, pos);
            LOG_ERROR("video_stream_input substr first");
            kvs_names.push_back(video_stream_input);
            std::cout << video_stream_input << std::endl;
            kvs_rtsp_url.erase(0, pos + delimiter.length());
        }
        LOG_ERROR("kvs_rtsp_url substr end");
        std::cout << kvs_rtsp_url << std::endl;
        rtsp_urls.push_back(kvs_rtsp_url);
    }
    ...
    for (int i = 0; i < rtsp_urls.size(); ++i) {
      ...
      //  string stream_name = base_stream_name + '_' + std::to_string(i);
        string stream_name = kvs_names[i].c_str();
